...
class PhotoshopCCEngine(sgtk.platform.Engine):
    ...
    def destroy_engine(self):
        ...
        if self._PROXY_WIN_HWND and sys.platform == "win32":
            self.__tk_adobecc.win_32_api.SetParent(self._PROXY_WIN_HWND, 0)
    ...
    def _win32_get_photoshop_main_hwnd(self):
        ...
        if not self._WIN32_PHOTOSHOP_MAIN_HWND:
            found_hwnds = self.__tk_adobecc.win_32_api.find_windows(
    ...
    def _win32_get_proxy_window(self):
        ...
        if ps_hwnd:
            ...
            proxy_win_hwnd = self.__tk_adobecc.win_32_api.qwidget_winid_to_hwnd(
            ...
            win_ex_style = self.__tk_adobecc.win_32_api.GetWindowLong(
                proxy_win_hwnd,
                self.__tk_adobecc.win_32_api.GWL_EXSTYLE,
            )

            self.__tk_adobecc.win_32_api.SetWindowLong(
                proxy_win_hwnd,
                self.__tk_adobecc.win_32_api.GWL_EXSTYLE, 
                win_ex_style | self.__tk_adobecc.win_32_api.WS_EX_NOPARENTNOTIFY,
            )

            # Parent to the Photoshop application window.
            self.__tk_adobecc.win_32_api.SetParent(proxy_win_hwnd, ps_hwnd)
